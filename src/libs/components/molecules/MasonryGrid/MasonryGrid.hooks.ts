...
export const useMasonryGrid = ({
  ...
  const assignInitialSpans = useCallback(
    (items: IMasonryItem[], columnCount: number, useRandomSpan: boolean) => {
  ...
  const adjustLastRow = useCallback(
    (items: { item: IMasonryItem; col: number }[], columnCount: number) => {
      let total = 0
